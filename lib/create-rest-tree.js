...
export function addTreeBranch(branch, record, indexPath, closurePack) {
  ...
  if (record.children && record.children.length) {
    ...
    record.children.forEach((childRecord, index) => addTreeBranch(branch[record.name], childRecord, indexPath.concat(index), closurePack));
    return;
  }
...
export function lazyCalcLeafNode(indexPath, closurePack) {
  return (props) => {
    ...
    const [propNamesPath, record] = calculateBranchNodes(records, indexPath, [], closurePack);
...
export function createExecFunc(record, fullName, axios) {
  ...
  const hooks = [].concat(record.hooks.beforeRequestHooks, [createRequestFunc()], record.hooks.afterRequestHooks);

  const fn = compose(hooks);

  return (props = {}) => {
    const tmpOptions = merge(record.options, parseExecArgs(record.options.url, props, record), { clone: true });
    const context = createContext(Object.assign({}, record.meta, { url_params: props.url_params }, props.meta), tmpOptions);
